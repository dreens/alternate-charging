...
def decelProg(vi,vf,select):
    if select == 'SF':
        dSeq = alternateDecelerator.delayModeAccel
    elif select == 'S=1':
        dSeq = alternateDecelerator.normalModeAccel
    dSeq.calcPhase(vi,vf)
    dSeq.calcTime()
#    dSeq.plot()
...
for v in vf:
    for mode in ['S=1','SF']:
        ...
        for d in delay:
            ...
            peak = (decelSeq.off+offset + 0.005/decelSeq.fvel)*pb.s
            #print peak
