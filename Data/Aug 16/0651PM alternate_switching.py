...
def decelProg(vi,vf,select):
    if select == 'SF':
        ...
        dSeq = alternateDecelerator.S3Mode
...
decelSeq = decelProg(810,400,'S=1')
...
files = {(v,'SF'):'SF Mode vS3 vf=%d' %(v) for v in vf}
#files.update({(v,'S=1'):'S=1 Mode vf=%d' %(v) for v in vf})
files.update({(v,'S=3'):'S=3 Mode vf=%d' %(v) for v in vf})
...
for v in vf:
    for mode in ['S=3','SF']:
        ...
        delay = yeutil.frange(0,full*2,full)
        for d in delay:
