...
At some minimum escape energy, the traveling well equipotential fails to be closed, enabling molecules with that amount of energy to escape. 
...
The minimum escape energies are plotted for all modes and as a function of varying decelerations in Fig.~\ref{fig:efftrap}a.
These energies are a useful figure of merit for characterizing the performance of a given operating mode, as evidenced by the close agreement between Fig.~\ref{fig:efftrap}a and Fig.~\ref{fig:efftrap}c, where the molecule number remaining in the traveling well after a $3$~ms hold time is reported.

For the S\,=\,1 mode, the minimum escape energies are nearly zero, particularly for molecules which move away from the trap center along the $x$ and $y$ axes. 
...
The minimum escape energy for S\,=\,1 actually improves with stronger deceleration, as expected given the increased sampling of the transversely focusing region when molecules climb further towards the charged pin pair, see Fig.~\ref{fig:chargecartoon}, Dist.~A.
...
Essentially it is required that the longitudinal velocity $v$ of the molecules satisfy $v/D >> f$, $D$ the stage distance and $f$ the oscillation frequency in the traveling well.
The precise dynamics as one approaches this bound are relevant, especially for trapping experiments, which necessarily violate this condition as $v\rightarrow 0$.
...
The asymptotically flat profiles at high enough speeds validate the traveling potential approach, as do the quantitative agreement between the asymptotic values and the corresponding points at $200\text{ km/s/s}$ and $67\text{ km/s/s}$ in Fig.~\ref{fig:efftrap}c.
...
This is a consequence of the tuning of an extra parameter afforded by the new deceleration modes- the relative time devoted to the original field distributions (A, A' in Fig.~\ref{fig:chargecartoon}) and those with increased transverse focusing (B--E in Fig.~\ref{fig:chargecartoon}).
In all operation mode diagrams shown in Fig.~\ref{fig:chargecartoon}, new field distributions are applied over a spatial region that is exactly centered around the grounded pin pair.
...
Instead, by addressing low escape energies from the traveling potential well, a greater flux of molecules at the same temperatures as before is obtained.
