...
To understand the success of this new strategy, we revisit the operating principle of a decelerator; refer to Fig.~\ref{fig:chargecartoon} where field distributions and the operating modes derived from them are detailed.
...
(a) $E_\text{esc}$ verse deceleration. 
...
The homogeneous initial phase space density assumed in Fig.~\ref{fig:phasespace} is only valid if the initial beam source generates a much broader distribution in phase space than the volume accepted by the traveling potential well.
...
Skimmer cooling addresses this~\cite{Wu2018}, and may be particularly well suited for pairing with SF and VSF mode.
...
Instead, by increasing the minimum escape energy $E_\text{esc}$ from the traveling potential well, we enhance the flux of molecules at the same temperatures as before.
