...
We introduce a new deceleration strategy that offers many-fold performance improvements across all useful operating conditions for a Stark decelerator, including ten-fold increases in molecule number at trappable final velocities.
...
Here we introduce a strategy that works on a conventional system, but fully resolves transverse challenges and offers more than tenfold gains even at very low speeds.
...
\begin{figure*}[t]
\includegraphics[width=\linewidth]{Configurations/pinpairformal.png}%
\caption{
This schematic illustrates a new strategy for Stark deceleration, which critically relies on certain distributions of the electric field, which may be generated by charging the decelerator electrodes in a novel way. Distributions B-D feature strong transverse focusing in the regions where molecules would normally pass between grounded pin pairs. On-axis energy diagrams are shown for several modes of operation incorporating these field distributions, with primes indicating translation to the next pin pair. In addition to S=1 mode and its S=3 overtone~\cite{VanDeMeerakker2005a}, a strong focusing (SF) and a very strong focusing (VSF) mode are introduced.\vspace{-4mm}
}
\label{fig:chargecartoon}
\end{figure*}

%\section{Alternate Charging Technique}
Our strategy is to mix new field distributions into the deceleration process that feature strong restoring force in the transverse directions, refer to Fig.~\ref{fig:chargecartoon}.
...
In the canonical S\,=\,1 deceleration strategy~\cite{VanDeMeerakker2012}, molecules approach a charged pin-pair, climbing a hill in potential energy. 
The hill is abruptly switched off, allowing molecules to then repeat the process without regaining that potential energy.
The abrupt switch must happen only partway up the potential energy hill, so that molecules that are ahead get more energy removed, and vice versa. 
This creates a traveling potential well for the molecules, with a longitudinal restoring force back to the center of a decelerating reference frame whose deceleration is set by the chirp rate of the switching frequency.
...
Conventionally, pins are always charged in bipolar pairs, in which case transverse focusing occurs right between the charged pin pair, but not significantly elsewhere.
This creates the well-known transverse-longitudinal coupling problem, wherein molecules experience much better transverse focusing when they are regularly sampling the focusing fields right between the charged pin-pair, which requires them to be oscillating with large amplitude in the longitudinal direction.
...
By implementing these distributions when the synchronous molecule is flying between the grounded pin pair, but retaining the use of the conventional distribution otherwise, the longitudinal behavior of the device is unaffected while the transverse behavior is vastly improved.
...
Utilizing the field distribution arising from charging only a single rod, something readily achievable with no change to high voltage electronics, gives rise to a focusing mode (F), and utilizing the distribution where two pins are both charged to the same voltage gives rise to strong focusing (SF). 
Charging all four pins, one pair at one voltage and the next pair at the opposite voltage, gives very strongly focusing (VSF).
...
Large gains persist even down to final speeds appropriate for trap loading, with SF outperforming S=1 at $50\text{ m/s}$ elevenfold.
...
We emphasize that to achieve F mode, no wiring changes are required relative to canonical operation, and the total number of state transitions driven by each high-voltage switch remains the same.
With some investment, we have also implemented the ability to run SF mode thanks to a liquid cooled tri-state switch capable of switching quickly and frequently between all three output states~\footnote{Behlke HTS-301-151-SiC, options HFB, ILC, ALL-OFF-BIPOLAR.}.
A second such switch would enable VSF mode, but we do not pursue this since it is less useful for trapping, our primary emphasis.
We find at least fourfold enhancements across a wide range of final speeds by using F mode, and tenfold enhancements for SF mode.
