...
Motivated by the holes evident in \SO\s mode, we introduce a new figure of merit that may be used to compare the performance of various modes of operation: the minimum depth of their effective moving traps.
...
If XSF mode were used with this $3$x$3\text{ mm}^2$ device~\cite{Scharfenberg2009} or this $4$x$4\text{ mm}^2$~\cite{VandeMeerakker2005} device, phase space volume would increase with the cube of pin pair spacing, and deceleration capability would reduce to linear order, thus significantly outperforming TW. 

The validity of using the effective trap in this manner depends however on the final speed after the deceleration sequence.
...
This can be attributed to the fact that XSF actually features an increased trap depth and trap frequency relative to the others, while SF and VSF improve over S=1 mostly by plugging holes and not by increasing the trap frequency.
One important corollary is that XSF may only be useful for trap loading with devices such as~\cite{Quintero-Perez2013} where a TW device is already going to be used for the final slowing.
