...
We begin by introducing a new metric for the performance of a decelerator and its operation mode: the minimum depth of its effective moving trap. 
The computation of the effective pendulum-like trapping force experienced by molecules in the restricted problem of their on-axis motion is completely standard~\cite{Bethlem2000,Hudson2004}, but the full 3D effective restoring force has not been reported previously for pulsed devices. 
We compute this in Appendix~\ref{app:effpot} and use it to numerically evaluate minimum depths over various decelerations and operation modes in Fig.~\ref{fig:efftrap}(a). Remarkably, SF mode, available immediately simply by turning off part of the decelerator in a clever manner, offers comparable trap depth improvement to S=3, but with no sacrifice in deceleration capability. The VSF and XSF modes make still more dramatic improvements, with the latter even rivaling traveling wave (TW) deceleration~\cite{}. Note that for VSF and XSF, the alternate configurations are not utilized in a symmetric manner about the grounded pin pair as for SF.

We can make further use of the effective trap by directly employing it to simulate the fate of particles confined within for a length of time equal to $3\text{ ms}$, the duration of a typical deceleration sequence. The results show a very close qualitative match to the trends predicted by the minimum trap depth. The most notable exception is found in S=1 mode at low decelerations, where extremely deep holes dominate the minimum trap depth, but the effective cross sectional area of the holes is rather small, see the equipotential surfaces in Fig.~\ref{fig:chargecartoon}. Therefore molecules can still survive in greater number than at higher decelerations where the minimum trap depth technically improves.

The validity of using the effective trap in this manner depends on the final speed after the deceleration sequence, as shown in Fig.~\ref{fig:efftrap}(c). At high enough speeds, it is fully valid, note the excellent agreement between the asymptotic values for all modes and the corresponding points at $200\text{ km/s/s}$ and $67\text{ km/s/s}$ in panel (b). The breakdown depends on the intended use of the decelerator, and especially how far the molecules will be expected to travel unguided afterwards. In panel (c), the molecules still confined within a $3\text{ mm}$ diameter circle after $5\text{ mm}$ free flight after the end of the sequence are shown. It is possible to do even better with a very closely mounted trap~\cite{aoeu}, but for collisional experiments a much larger flight distance may be required.
...
Equipotential surfaces for these effective traps are also shown in Fig.~\ref{fig:chargecartoon}. It is found that the worst-case depth for S=1 is in fact incredibly small. In particular, molecules that deviate transversely from the synchronous molecule along the $x$ and $y$ axes experience almost no trap at all. 
This can be considered the underlying reason for the transverse-longitudinal coupling problem that has been described~\cite{VanDeMeerakker2006}. Such couplings are in some contexts useful for maintaining ergodicity in a trapping geometry~\cite{Surkov1996}, but with one dimension featuring a very low energy barrier, they lead to unwanted loss.

To address this, we mix alternate charge configurations into the deceleration scheme that feature strong restoring force in the transverse directions. 
...
In the canonical operating mode~\cite{VanDeMeerakker2012}, the average voltage of the charged pin-pair is zero, and few field lines run toward the adjacent grounded pin-pair (Fig.~\ref{fig:chargecartoon}). 
...
These alternate configurations can be implemented when the synchronous molecule is flying between the grounded pin pair, so that the longitudinal behavior is not affected.
Including the alternate configurations corresponding to having only a single rod charged gives rise to the mode we term "strong focusing" (SF), and including the configuration where both pins are charged to the same voltage gives rise to "very strong focusing" (VSF).

We also note the dramatic reduction in asymmetry afforded by these alternate configurations, especially as compared with traveling wave Stark deceleration.
This can be attributed to the linear verse quadratic character of the effective moving trap potential in the traveling wave and pulsed devices, respectively. 
The former becomes dramatically asymmetric when tilted by a deceleration force, while the latter is only shifted in its zero.
